...
FilmTiler::FilmTiler()
{
	...
	AddParam( vtiles = ParamRange::Create( "Vertical Tiles", 3.0f, { 1.0f, 10.0f } ) );
	AddParam( htiles = ParamRange::Create( "Horizontal Tiles", 1.0f, { 1.0f, 10.0f } ) );
	AddParam( tilex = Param::Create( "Size", 0.25f ) );
	AddParam( offsetx = Param::Create( "X Offset", 0.1f ) );
	AddParam( offsety = Param::Create( "Y Offset", 0.1f ) );
...
FFResult FilmTiler::InitGL( const FFGLViewportStruct* vp )
{
...
FFResult FilmTiler::ProcessOpenGL( ProcessOpenGLStruct* pGL )
{
	...
	shader.Set( "HTiles", int( htiles->GetValue() ) );
